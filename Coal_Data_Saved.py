...
def log_model_evidence(param, *args):  # Param includes both sigma and l, arg is passed as a pointer
    ...
    c_auto = matern(3/2, sigma, length, x_data, x_data)
...
log_likelihood = log_model_evidence(solution.x, *xy_data)
...
C_dd = matern(3/2, sigma_optimal, length_optimal, x, x)
...
for i in range(sampling_points.size):
    ...
    C_star_d = matern(3/2, sigma_optimal, length_optimal, x_star, x)
    C_star_star = matern(3/2, sigma_optimal, length_optimal, x_star, x_star)
...
Prediction.set_title('Posterior Distribution(Matern 3/2)')
