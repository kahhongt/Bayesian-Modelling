...
sampling_points_x = np.linspace(np.min(xv_transform_row), np.max(xv_transform_row), intervals)
sampling_points_y = np.linspace(np.min(yv_transform_row), np.max(yv_transform_row), intervals)
...
C_dd = squared_exp_2d(sigma_optimal, length_optimal, xy_data_coord, xy_data_coord)
...
for i in range(sampling_coord.shape[1]):
    ...
    C_star_d = squared_exp_2d(sigma_optimal, length_optimal, xy_star, xy_data_coord)
    C_star_star = squared_exp_2d(sigma_optimal, length_optimal, xy_star, xy_star)
...
data_transform = fig_pp_data.add_subplot(223)  # Transformed Data Scatter
...
bin_plot = fig_pp_data.add_subplot(224, projection='3d')
